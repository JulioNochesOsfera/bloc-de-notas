...
<nav class="navbar navbar-expand-lg navbar-dark bg-dark">
  <div class="container-fluid">
    ...
    <div class="container-sm mt-4 pt-4 pb-4 ">
        <h2>Nota: {{$dato->id}}</h2>
        <form action="{{route('editar',$dato->id)}}" method="POST" class="ms-5 me-5">
            @csrf
            @method('PUT')
            @if(session('mensaje'))
            ...
            <div class="mb-3 ">
                ...
                <input type="text" name="titulo" class="form-control" placeholder="Ingresa Titulo" value="{{$dato->titulo}}">
            </div>
            <div class="mb-3 bottom ">
                <label class="form-label">Texto</label>
                <textarea class="form-control" name="descripcion" rows="3">{{$dato->descripcion}}</textarea>
            </div>
